...
public class BestFirstTest {
    ...
    /**
     * BestFirst between any two random nodes
     * time is inaccurate due to filtering
     */
    @Test
    public void randomPathAny(){
        Context context = new Context(new BestFirst(), _Initialize.getEdgeHashMap(),
    ...
    /**
     * BestFirst between any two random nodes on the same floor
     * time is inaccurate due to filtering
     */
    @Test
    public void randomPathSameFloor(){
        ...
        Context context = new Context(new BestFirst(), _Initialize.getEdgeHashMap(), node1.getNodeID(), node2.getNodeID(), 1);
        double[][] coords = context.runOne();
        assertNotEquals(0, coords.length);
    ...
    /**
     * BestFirst between any two random nodes on a different floor
     * time is inaccurate due to filtering
     */
    @Test
    ...
    /**
     * BestFirst between the same node
     */
    @Test
    public void randomPathSameNode(){
        Node node1 = (Node) RandomObject.retrieveRandom("Node");
        Context context = new Context(new BestFirst(), _Initialize.getEdgeHashMap(), node1.getNodeID(), node1.getNodeID(), 1);
        double[][] coords = context.runOne();
